...
!function (g) {
  ...
  r2 = {
    ...
    // 这里面的i2才是我们要用的，直接搞个全局变量装起来
    i2.util.global.protobuf = i2,
    commonjsGlobal = i2;
  module && module.exports && (module.exports = i2);
}();

// console.log(commonjsGlobal);

/**
 * 方案
 * F12查看请求头，在调用栈里面跟进promise.then之后的代码，如下所示，这里的e里面请求头是加密前的，
 * 跟进去i，看i的每个回调方法，加上断点看在那个方法里面进行了加密，
 * 方法代码很多每次都在代码中间的位置打上断点依次判断，最终的加密方法是什么
 * r = Promise.resolve(e);
 * r = r.then(i.shift(), i.shift());
 * 最终加密就是这行代码改变的
 * l[r("xQep", 1253, 1146, 986) + "e"](i[c(451, 901, "p0cj", 555)])[u(895, "1x2h", 1541, 1211, 1281) + "h"]()[c(-225, -721, "yTXu", -430)]()
 *
 * 混淆代码还原，例如：r("xQep", 1253, 1146, 986) + "e" ==> encode
 * 还原之后的代码就是： l['encode'](i['data'])['finish']()['slice']()
 *
 * l['encode'] ==> PolicyInfoByTypeIdParam_encode
 * i['data'] ===> params  也就是明文请求体
 *
 * */
...
function PolicyInfoByTypeIdParam_encode(m, w) {
  if (!w)
    // 本来这个的Writer就在commonjsGlobal里面了
...
// console.log(PolicyInfoByTypeIdParam_encode(params)['finish']()['slice']());

// 封装这个方法给py调
function getParams(params){
    return PolicyInfoByTypeIdParam_encode(params)['finish']()['slice']()
}
