 ...
 * 反向解析X-s的参数
 * @author modify
 * 1、获取X-s参数，直接全局搜索X-s（区分大小写）
 * 2、核心在于_webmsxyw方法的加密，这段代码是混淆的，可以使用js混淆还原：https://github.com/kuizuo/js-deobfuscator。同目录下的output.js就是还原后的代码
 * */
...
/*
v = (u && void 0 !== window._webmsxyw ? window._webmsxyw : ep)(d, s) || {};
// 在控制台直接调用window._webmsxyw(d,s)和ep(d,s)比较，发现window._webmsxyw生成的X-s和接口调用是一样的
